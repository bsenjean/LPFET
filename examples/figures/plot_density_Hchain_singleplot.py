...
# assuming the same list_R is used for any file...
for R_index in range(len(list_R[0])):
   ...
   ax.plot(np.array([i for i in range(6)]), list_n_FCI[0][R_index], color="black", ls='-', label="FCI")
   for i in range(len(file_name)):
     ax.plot( np.array([i for i in range(6)]), list_n_cluster[i][R_index], color=colorFader("red","blue",i/len(file_name)), ls='--', marker=i+2, label=file_name[i], markersize=15)
     ax.plot( np.array([i for i in range(6)]), list_n_KS[i][R_index], color=colorFader("red","blue",i/len(file_name)), ls=':', marker=i+2, markersize=15)
   ax.legend(framealpha=0.8, prop={'size': 20}, loc="best")
   plt.tight_layout()
   plt.savefig('density_Hchain_R{}.pdf'.format(list_R[0][R_index]))
