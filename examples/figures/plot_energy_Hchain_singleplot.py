...
ax.plot( list_R[0], list_E_FCI[0], color="black", ls='-', label="FCI")
for i in range(len(file_name)):
  ax.plot( np.asarray(list_R[i]), list_E[i], color=colorFader("red","blue",i/len(file_name)), ls='--', marker=i+2, label=file_name[i], markersize=15)
