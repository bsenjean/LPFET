...
ax.plot( np.divide(np.asarray(list_U[0]),np.asarray(list_U[0]) + 4), list_E_FCI[0], color="black", ls='-', label="FCI")
for i in range(len(file_name)):
  ax.plot( np.divide(np.asarray(list_U[i]),np.asarray(list_U[i]) + 4), list_E[i], color=colorFader("red","blue",i/len(file_name)), ls='--', marker=i+2, label=file_name[i], markersize=15)
