...
def norm_density(params):
    ...
    for impurity_index in range(N_mo):
        ...
        H_cl = tools.build_hamiltonian_quantum_chemistry(h_cl_core - np.diag([v_Hxc[impurity_index], 0]), g_cl_core, basis_cl, a_dag_a_cl )
...
U_list=[0.0,1.0,2.0,3.0,5.0,7.0,10.0,15.0,30.0,50.0,100.0]
basin_hopping = False
...
f = open(output,'w')
f.write("{:>12s} {:>16s} {:>16s} ".format("U","FCI_energy","energy"))
...
for U in U_list:
    ...
    for i in range(N_mo): g[i,i,i,i] = U
    ...
    f = open(output,'a')
    f.write("{:>12.6f} {:>16.9f} {:>16.9f} ".format(U,E_FCI[0],sum_site_energy))
...
#    plt.savefig("./figures/density_U{:.3f}_ring_{}_{}_LPFET_KScluster.pdf".format(U_list[i],opt_method,init), format="pdf", bbox_inches="tight")
#    plt.show()
#
...
#plt.xticks(fontsize=17)
#plt.yticks(fontsize=17)
#plt.savefig("./figures/Hubbard_ring_energy_{}_{}_LPFET_KScluster.pdf".format(opt_method,init), format="pdf", bbox_inches="tight")
#plt.show()
