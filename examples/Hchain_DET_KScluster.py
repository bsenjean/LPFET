...
def norm_density(params):
    ...
    for impurity_index in range(N_mo):
        ...
        h_Ht, g_Ht = tools.transform_1_2_body_tensors_in_new_basis( h_KS_permuted, g_permuted, C_ht )
        ...
        H_cl = tools.build_hamiltonian_quantum_chemistry(h_cl_core - np.diag([mu,0]), g_cl_core, basis_cl, a_dag_a_cl )
...
initial_guess = np.zeros(N_mo//2+1)
Distance=[0.4,0.6,0.8,1.0,1.5,2.5]
...
output = 'results/{}.out'.format(file_name)
f = open(output,'w')
f.write("{:>12s} {:>16s} {:>16s} ".format("R","FCI_energy","energy"))
f.writelines("{:>16s} ".format("n{}_FCI".format(site)) for site in range(N_mo))
f.writelines("{:>16s} ".format("n{}_cluster".format(site)) for site in range(N_mo))
f.writelines("{:>16s} ".format("n{}_KS".format(site)) for site in range(N_mo))
f.writelines("{:>16s} ".format("vHxc{}".format(site)) for site in range(N_mo))
f.write("{:>16s} ".format("N_elec"))
f.write("{:>16s} ".format("mu"))
f.write("\n")
f.close()
...
for R in Distance:
    ...
    h, g = tools.transform_1_2_body_tensors_in_new_basis(h_AO, g_AO, S_half)
    ...
    print(Optimized)
    v_Hxc = np.zeros(2*(len(Optimized.x[:-1])))
    for i in range(len(Optimized.x[:-1])):
    ...
    f = open(output,'a')
    f.write("{:>12.6f} {:>16.9f} {:>16.9f} ".format(R,E_FCI[0]+E_nuc,sum_site_energy + E_nuc))
    f.writelines("{:>16.9f} ".format(i) for i in FCI_density)
    f.writelines("{:>16.9f} ".format(i) for i in occ_cluster)
    f.writelines("{:>16.9f} ".format(i) for i in occ_KS)
    f.writelines("{:>16.9f} ".format(i) for i in v_Hxc)
    f.write("{:>16.9f} ".format(2*sum(occ_cluster)))
    f.write("{:>16.9f} ".format(mu))
    f.write("\n")
    f.close()
...
#    plt.savefig("./figures/density_R{:.3f}_{}_DET_KScluster.pdf".format(Distance[i],opt_method), format="pdf", bbox_inches="tight")
#    plt.show()
#
...
#plt.xticks(fontsize=17)
#plt.yticks(fontsize=17)
#plt.savefig("./figures/Hchain_energy_{}_DET_KScluster.pdf".format(opt_method), format="pdf", bbox_inches="tight")
#plt.show()
#
